            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody id="tableBody">
                    ...
                    <tr>
                        ...
                        <td align="right">{{ $data_value->kuantiti }}</td>
                        <td align="center"><a class="btn btn-danger btn-sm" onclick="deleteData({{ $data_value->id }})">Hapus</a> 
                            <!-- <a class="btn btn-warning btn-sm" id="" onclick="formUpdate({{ $data_value->id }})">update</a> -->
                        </td>
                    </tr>
                    @php $no++; @endphp
                    @endforeach
                    ...
                    <div>
                        <div class="form-group">
                            ...
                            <select name="kode_barang"  id="kode_barang"  class="form-control">
                                ...
                                <option value="{{ $items_value->kode_barang }}">{{ $items_value->nama_barang }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label >Kuantiti</label>
                            <input type="test" class="form-control" id="kuantiti" placeholder="Kuantiti">
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-dialog modal-md">
                ...
                <div class="modal-body">
                    ...
                    <div>
                        <div class="form-group">
                            <label >Nama_barang</label>
                            <select name="kode_barang_update"  id="kode_barang_update"  class="form-control">
                                <option value="">Pilih</option>
                                @foreach($items as $items_value)
                                <option value="{{ $items_value->kode_barang }}">{{ $items_value->nama_barang }}</option>
                                @endforeach
                        ...
                        <div class="form-group">
                            <label >Kuantiti</label>
                            <input type="text" class="form-control" id="kuantiti_update" placeholder="Kuantiti">
        ...
        <script>
            $(document).ready(function(){
                ...
                $(`#btnSave`).on("click", function(){
                    $.ajax({
                        url: "{{ url('/save_data') }}",
                        data: `kuantiti=${$(`#kuantiti`).val()}&kode_barang=${$(`#kode_barang`).val()}`,
                        type: 'POST',
                        dataType: 'json',
                ...
                $(`#btnUpdate`).on("click", function(){
                    $.ajax({
                        url: "{{ url('/update_data') }}",
                        data: `kuantiti=${$(`#kuantiti_update`).val()}&kode_barang=${$(`#kode_barang_update`).val()}&id=${$(`#id_update`).val()}`,
                        type: 'POST',
                        dataType: 'json',
                        success: function (json) {
                            if(json.status_code ==1)
            ...
            function getDataTable(dataTable)
            {
                ...
                for(let i=0; i<dataTable.length; i++)
                {
                    ...
                    //     <td align="right">${dataTable[i].kuantiti}</td>
                    ...
                    html +=`<tr>
                        ...
                        <td >${dataTable[i].kode_barang}</td>
                        <td align="right">${dataTable[i].kuantiti}</td>
                        <td align="center"><a class="btn btn-danger btn-sm" onclick=deleteData(${dataTable[i].id})>Hapus</a> </td>
                    </tr>`                    
                    no++
                }
                return html;
            ...
            function formUpdate(id)
            {
                ...
                $.ajax({
                        ...
                        dataType: 'json',
                        success: function (json) {
                            $("#kuantiti_update").val(json.data.kuantiti)
                            $("#id_update").val(json.data.id)
                            $(`#kode_barang_update option[value=${json.data.kode_barang}]`).attr('selected','selected');
